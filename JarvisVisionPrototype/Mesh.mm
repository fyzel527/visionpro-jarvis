...
TexturedMesh::TexturedMesh(MDLMesh *mdlMesh, NSString *imageName, id<MTLDevice> device) {
    ...
    _texture = imageName ? CreateTextureFromImage(imageName, device, &error) : nil;
...
void TexturedMesh::draw(id<MTLRenderCommandEncoder> renderCommandEncoder, PoseConstants *poseConstants, size_t poseCount) {
    ...
    instanceConstants.sceneTime = sceneTime();
    instanceConstants.padding[0] = instanceConstants.padding[1] = instanceConstants.padding[2] = 0.0f;
    ...
    [renderCommandEncoder setFragmentBytes:&instanceConstants length:sizeof(instanceConstants) atIndex:0];
